...
public class UserDataset implements Dataset {
  ...
  @Override
  public void load(RecordLayer recordLayer, String tenant, String recordSpace, int nbrRecord)
  ...
  public DemoUserProto.User createUser(long id, Faker faker) {
    ...
    favoritePlanets.put(
        "hitchhikers_guide_to_the_galaxy", faker.hitchhikersGuideToTheGalaxy().planet());
    favoritePlanets.put("rick_and_morty", faker.rickAndMorty().location());
    favoritePlanets.put("star_trek", faker.starTrek().location());

    DemoUserProto.Address address =
        DemoUserProto.Address.newBuilder()
