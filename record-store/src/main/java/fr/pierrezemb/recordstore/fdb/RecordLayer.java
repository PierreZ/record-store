...
public class RecordLayer {
  ...
  public RecordLayer(String clusterFilePath, boolean enableMetrics, SecretKey key)
  ...
  public List<String> listContainers(String tenantID) {
    ...
    List<ResolvedKeySpacePath> recordSpaces =
        tenantKeySpace.listSubdirectory(context, "recordSpace", ScanProperties.FORWARD_SCAN);
  ...
  public void deleteContainer(String tenantID, String recordSpace) {
    FDBRecordContext context = db.openContext(Collections.singletonMap("tenant", tenantID), timer);
    FDBRecordStore.deleteStore(
        context, RecordStoreKeySpace.openDataKeySpacePath(tenantID, recordSpace));
    FDBRecordStore.deleteStore(
        context, RecordStoreKeySpace.openMetaDataKeySpacePath(tenantID, recordSpace));
    context.commit();
  }
  ...
  public RecordMetaData getSchema(String tenantID, String recordSpace) {
    ...
    FDBMetaDataStore metaDataStore =
        RecordStoreMetaDataStore.createMetadataStore(context, tenantID, recordSpace);

    List<RecordStoreProtocol.IndexDescription> indexes =
        metaDataStore.getRecordMetaData().getAllIndexes().stream()
            .filter(e -> !e.getName().startsWith("global"))
            .map(e -> RecordStoreProtocol.IndexDescription.newBuilder().build())
            .collect(Collectors.toList());

    return metaDataStore.getRecordMetaData();
  }

  public List<RecordStoreProtocol.IndexDescription> getIndexes(
      String tenantID, String recordSpace) {
    FDBRecordContext context = db.openContext(Collections.singletonMap("tenant", tenantID), timer);
    FDBMetaDataStore metaDataStore =
        RecordStoreMetaDataStore.createMetadataStore(context, tenantID, recordSpace);
  ...
  public void upsertSchema(
      String tenantID,
      String recordSpace,
      DescriptorProtos.FileDescriptorSet schema,
      List<RecordStoreProtocol.RecordTypeIndexDefinition> indexes)
    ...
    try {
      ...
      LOGGER.debug(
          "metadata for {}:{} is in version {}", tenantID, recordSpace, oldMetaData.getVersion());
    ...
    // handling upgrade
    if (null != oldMetaData) {
      MetaDataEvolutionValidator metaDataEvolutionValidator =
  ...
  private RecordMetaData createRecordMetaData(
    ...
    HashSet<Index> oldIndexes =
        oldMetadata != null ? new HashSet<>(oldMetadata.getAllIndexes()) : new HashSet<>();
    ...
    for (Index index : oldIndexes) {
      ...
      if (index.getName().equals(UniversalIndexes.COUNT_INDEX_NAME)
          || index.getName().equals(UniversalIndexes.COUNT_UPDATES_INDEX_NAME)) {
    ...
    for (RecordStoreProtocol.RecordTypeIndexDefinition idxRequest : indexes) {
      ...
      // add new indexes
      for (RecordStoreProtocol.IndexDefinition indexDefinition :
          idxRequest.getIndexDefinitionsList()) {
      ...
      // set primary key
      metadataBuilder
          .getRecordType(idxRequest.getName())
  ...
  private String generateIndexName(
      String name, RecordStoreProtocol.IndexDefinition indexDefinition) {
    if (!indexDefinition.hasNestedIndex()) {
      return name
          + "_idx_"
          + indexDefinition.getField()
          + "_"
          + indexDefinition.getIndexType().toString();
    }
    return name
        + "_idx_"
        + indexDefinition.getField()
        + "_nested_"
        + generateIndexName(name, indexDefinition.getNestedIndex());
  }
  ...
  private Index createIndex(RecordStoreProtocol.IndexDefinition indexDefinition, String indexName) {
    ...
    if (indexDefinition.hasNestedIndex()) {
      return new Index(
          ...
          Key.Expressions.field(
                  indexDefinition.getField(), getFanType(indexDefinition.getFanType()))
    ...
    switch (indexDefinition.getIndexType()) {
      ...
      case TEXT_DEFAULT_TOKENIZER:
        ...
        break;
      case VERSION:
        index = new Index(indexName, VersionKeyExpression.VERSION, IndexTypes.VERSION);
        break;
      case MAP_KEYS:
        index = new Index(indexName, Key.Expressions.mapKeys(indexDefinition.getField()));
        break;
      case MAP_VALUES:
        index = new Index(indexName, Key.Expressions.mapValues(indexDefinition.getField()));
        break;
      case MAP_KEYS_AND_VALUES:
        index = new Index(indexName, Key.Expressions.mapKeyValues(indexDefinition.getField()));
        break;
      case UNRECOGNIZED:
        return null;
    }
  ...
  private KeyExpression createKeyExpressionFromIndexDefinition(
      RecordStoreProtocol.IndexDefinition nestedIndex) {
  ...
  private KeyExpression buildPrimaryKeyExpression(List<String> primaryKeyFields) {
    List<KeyExpression> keyExpressions =
        primaryKeyFields.stream().map(Key.Expressions::field).collect(Collectors.toList());
  ...
  public Tuple getCountAndCountUpdates(String tenantID, String recordSpace, SecretKey key) {
    ...
    CompletableFuture<Tuple> countFuture =
        r.evaluateAggregateFunction(
    ...
    CompletableFuture<Tuple> updateFuture =
        r.evaluateAggregateFunction(
            EvaluationContext.EMPTY,
            Collections.emptyList(),
    ...
    return countFuture
        .thenCombine(
            updateFuture, (count, update) -> Tuple.from(count.getLong(0), update.getLong(0)))
        .join();
  }

  public void putRecord(
      String tenantID, String recordSpace, String table, byte[] record, SecretKey customKey)
    ...
    Descriptors.Descriptor descriptor =
        metaDataStore.getRecordMetaData().getRecordsDescriptor().findMessageTypeByName(table);
    ...
    FDBRecordStore r =
        createFDBRecordStore(context, metaDataStore, customKey, tenantID, recordSpace);
  ...
  public void putRecord(String tenantID, String recordSpace, String table, byte[] record)
      throws InvalidProtocolBufferException {
  ...
  public void putRecord(
    ...
    FDBRecordContext context = db.openContext(Collections.singletonMap("tenant", tenantID), timer);
    FDBRecordStore r =
        createFDBRecordStore(
            context, recordMetaData, secretKey, tenantID, managedSchemaType, managedSchema);

    LOGGER.info(
        "saving {} for {}/{}/{}",
        r.saveRecord(message).toString(),
        tenantID,
        managedSchemaType,
        managedSchema);
    context.commit();
  ...
  public List<Message> scanRecords(
      String tenantID,
      String managedSchemaType,
      String managedSchema,
      RecordMetaData recordMetaData,
      RecordQuery query) {
    return scanRecords(
        tenantID, managedSchemaType, managedSchema, recordMetaData, query, defaultKey);
  }
  ...
  public boolean deleteRecord(
      ...
      Tuple primaryKey) {
    return deleteRecord(
        tenantID, managedSchemaType, managedSchema, recordMetaData, primaryKey, defaultKey);
  }
  ...
  public List<Message> queryRecords(
      String tenantID, String recordSpace, RecordQuery query, SecretKey key) {
    FDBRecordContext context = db.openContext(Collections.singletonMap("tenant", tenantID), timer);
    FDBMetaDataStore metadataStore =
        RecordStoreMetaDataStore.createMetadataStore(context, tenantID, recordSpace);
    FDBRecordStore r = createFDBRecordStore(context, metadataStore, key, tenantID, recordSpace);
        ...
        .map(
            e -> {
              if (LOGGER.isTraceEnabled()) {
                LOGGER.trace(
                    "found record '{}' from {}/{}", e.getPrimaryKey(), tenantID, recordSpace);
              }
              return e;
            })
        .map(FDBRecord::getRecord)
  ...
  public void queryRecords(
      ...
      StreamObserver<RecordStoreProtocol.QueryResponse> responseObserver) {
  ...
  public void queryRecords(
    ...
    FDBRecordContext context = db.openContext(Collections.singletonMap("tenant", tenantID), timer);
    FDBMetaDataStore metadataStore =
        RecordStoreMetaDataStore.createMetadataStore(context, tenantID, container);
    ...
    this.executeQuery(r, query, isolationLevel)
        .map(
            e -> {
              if (LOGGER.isTraceEnabled()) {
                LOGGER.trace(
                    "found record '{}' from {}/{}", e.getPrimaryKey(), tenantID, container);
              }
              return e;
            })
        .map(FDBRecord::getRecord)
        ...
        .forEach(
            e ->
                responseObserver.onNext(
                    RecordStoreProtocol.QueryResponse.newBuilder().setRecord(e).build()))
        .join();
  }
  ...
  public void queryRecords(
      ...
      String container,
      RecordQuery query,
      Promise<List<Map<String, Object>>> future) {
  ...
  public void queryRecords(
    ...
    FDBMetaDataStore metadataStore =
        RecordStoreMetaDataStore.createMetadataStore(context, tenantID, container);
    FDBRecordStore r =
        createFDBRecordStore(context, metadataStore, encryptionKey, tenantID, container);

    Descriptors.Descriptor descriptor =
        r.getRecordMetaData().getRecordsDescriptor().findMessageTypeByName("User");

    List<Map<String, Object>> result = null;
    try {
      result =
          this.executeQuery(r, query)
              .map(
                  e -> {
                    if (LOGGER.isTraceEnabled()) {
                      LOGGER.trace(
                  ...
                  queriedRecord -> {
                    try {
                      return DynamicMessage.parseFrom(
                          descriptor, queriedRecord.getStoredRecord().getRecord().toByteArray());
              ...
              // TODO: can we replace `graphql.schema.PropertyDataFetcher` to avoid casting things
              // as an HashMap?
              .map(
                  dynamicMessage -> {
                    ...
                    dynamicMessage
                        .getAllFields()
                        .forEach((key, value) -> results.put(key.getName(), value));
                    return results;
                  })
              .asList()
              .get();
  ...
  private RecordCursor<FDBQueriedRecord<Message>> executeQuery(
      FDBRecordStore r, RecordQuery query) {
    return this.executeQuery(r, query, IsolationLevel.SERIALIZABLE);
  }

  private RecordCursor<FDBQueriedRecord<Message>> executeQuery(
      FDBRecordStore r, RecordQuery query, IsolationLevel isolationLevel) {
    ...
    ExecuteProperties.Builder executeProperties =
        ExecuteProperties.newBuilder()
            .setIsolationLevel(isolationLevel)
            .setDefaultCursorStreamingMode(
                CursorStreamingMode.ITERATOR); // either WANT_ALL OR streaming mode
  ...
  private FDBRecordStore createFDBRecordStore(
    ...
    TransformedRecordSerializer<Message> serializer =
  ...
  private FDBRecordStore createFDBRecordStore(
    ...
    return FDBRecordStore.newBuilder()
        ...
        .setSerializer(serializer)
        .setKeySpacePath(
            ManagedSchemaKeySpace.openDataKeySpacePath(tenantID, managedSchemaType, managedSchema))
        .createOrOpen();
  }
}
