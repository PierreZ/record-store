...
public class SchemaAdminServiceTest extends AbstractFDBContainer {
  ...
  public void testCRUDSchema2(Vertx vertx, VertxTestContext testContext) throws Exception {
    schemaServiceVertxStub.get(
        RecordStoreProtocol.GetSchemaRequest.newBuilder().setRecordTypeName("User").build(),
        response -> {
          if (response.succeeded()) {
            System.out.println("Got the server response: " + response.result());
            testContext.completeNow();
        ...
        RecordStoreProtocol.UpsertSchemaRequest.newBuilder()
            .addRecordTypeIndexDefinitions(
                RecordStoreProtocol.RecordTypeIndexDefinition.newBuilder()
                    ...
                    .addPrimaryKeyFields("id")
                    .addIndexDefinitions(
                        RecordStoreProtocol.IndexDefinition.newBuilder().setField("name").build())
                    .build())
            .setSchema(dependencies)
            .build();
  ...
  public void testCRUDSchema6(Vertx vertx, VertxTestContext testContext) throws Exception {
    adminServiceVertxStub.list(
        RecordStoreProtocol.ListContainerRequest.newBuilder().build(),
        response -> {
          if (response.succeeded()) {
            System.out.println("Got the server response: " + response.result());
            testContext.completeNow();
  ...
  public void testCRUDSchema7(Vertx vertx, VertxTestContext testContext) throws Exception {
    adminServiceVertxStub.delete(
        RecordStoreProtocol.DeleteContainerRequest.newBuilder()
