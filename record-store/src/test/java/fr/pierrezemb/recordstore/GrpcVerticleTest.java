...
public class GrpcVerticleTest extends AbstractFDBContainer {
  ...
  void deploy_verticle(Vertx vertx, VertxTestContext testContext) {
    ...
    schemaServiceVertxStub =
        SchemaServiceGrpc.newVertxStub(channel).withCallCredentials(credentials);
    recordServiceVertxStub =
        RecordServiceGrpc.newVertxStub(channel).withCallCredentials(credentials);
  }
  ...
  public void testCreateSchema(Vertx vertx, VertxTestContext testContext) throws Exception {
    ...
    RecordStoreProtocol.UpsertSchemaRequest request =
        RecordStoreProtocol.UpsertSchemaRequest.newBuilder()
  ...
  public void testPut2(Vertx vertx, VertxTestContext testContext) throws Exception {
    RecordStoreProtocol.StatRequest recordRequest =
        RecordStoreProtocol.StatRequest.newBuilder().build();

    schemaServiceVertxStub.stat(
        recordRequest,
        response -> {
          if (response.succeeded()) {
            System.out.println("Got the server response: " + response.result());
  ...
  public void testPut3(Vertx vertx, VertxTestContext testContext) throws Exception {
    ...
    RecordStoreProtocol.QueryFilterNode query =
        RecordStoreProtocol.QueryFilterNode.newBuilder()
            .setFieldNode(
  ...
  public void testPut4(Vertx vertx, VertxTestContext testContext) throws Exception {
    ...
    RecordStoreProtocol.QueryFilterAndNode andNode =
                ...
                RecordStoreProtocol.QueryFilterNode.newBuilder()
                    .setFieldNode(
                        RecordStoreProtocol.QueryFilterFieldNode.newBuilder()
                            ...
                            .setInt64Value(1)
                            .setOperation(
                                RecordStoreProtocol.FilterOperation.GREATER_THAN_OR_EQUALS)
                            .build())
                    .build())
            .build();

    RecordStoreProtocol.QueryFilterNode query =
        RecordStoreProtocol.QueryFilterNode.newBuilder().setAndNode(andNode).build();
  ...
  public void testPut5(Vertx vertx, VertxTestContext testContext) throws Exception {
    ...
    RecordStoreProtocol.DeleteRecordRequest request =
        RecordStoreProtocol.DeleteRecordRequest.newBuilder()
            .setFilter(query)
            .setRecordTypeName("User")
            .build();

    recordServiceVertxStub.delete(
        request,
        response -> {
          if (response.succeeded()) {
            System.out.println("Got the server response: " + response.result());
  ...
  public void testPut7(Vertx vertx, VertxTestContext testContext) throws Exception {
    ...
    RecordStoreProtocol.QueryRequest request =
        RecordStoreProtocol.QueryRequest.newBuilder()
            .setRecordTypeName("User")
            .setSortBy(
                RecordStoreProtocol.SortByRequest.newBuilder()
                    .setType(RecordStoreProtocol.SortByType.SORT_BY_NEWEST_VERSION_FIRST)
                    .build())
            .build();
